...
class EditDist:
    def __init__(self, word1: str, word2: str):
        self.word1 = word1
        self.word2 = word2
        # Initialize 2D matrices to all 0's
        # Distance Matrix is used to calculate the edit distance
        # Alignment Matrix is used for printing the alignment
        self._distanceMatrix = [[0 for _ in range(len(word1) + 1)] for _ in range(len(word2) + 1)]
        self._alignmentMatrix = [[0 for _ in range(len(word1) + 1)] for _ in range(len(word2) + 1)]
    ...
    def calculate(self):
        ...
        for i in range(1, len(self.word1) + 1):
            for j in range(1, len(self.word2) + 1):
                ...
                values = [self._distanceMatrix[j - 1][i] + 1, self._distanceMatrix[j][i - 1] + 1,
                          self._distanceMatrix[j - 1][i - 1] + 1]
